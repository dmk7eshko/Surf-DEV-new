...
//   });
  ...
  jQuery('.bgs-dev>ul>li>a').click(function() {
    jQuery(this).parent().find('.mega-menu').slideToggle();
    jQuery(this).toggleClass('active');
  });
	
  jQuery('.contact-form-input input, .contact-form-input textarea').click(function() {
    ...
    $toggle.click(function () {
      var template = function (post) {
        return $(
          '<article id="post-' +
          post.ID +
          '" class="blog-post col-4"onclick="window.location=`' +
          post.permalink +
		  '`;"><section class="blog-post-image"><a href="' +
          post.permalink +
          '"><img src="' +
          post.image +
          '" alt="' +
          post.post_title +
          '" /></a></section><section class="blog-post-content"><div class="blog-post_meta"><a class="blog-post-category" href="' +
          post.category.url +
          '">' +
          post.category.name +
          '</a>' +
			post.reading_time + 
		  ' MIN READ</div><h2><a href="' +
          post.permalink +
          '">' +
          post.post_title + post.reading +
        ...
        success: function (response) {
          ...
          response.forEach(function (item, index) {
			  ...
			  if(index === 0){
				  $blogposts.append(
				  $(
          '<div class="first-post"><article id="post-' +
          item.ID +
          '" class="blog-post row" onclick="window.location=`' +
          item.permalink +
		  '`;"><section class="blog-post-image col-8"><a href="' +
          item.permalink +
          '"><img src="' +
          item.image +
          '" alt="' +
          item.post_title +
          '" /></a></section><section class="blog-post-content col-4"><div class="blog-post_meta"><a class="blog-post-category" href="' +
          item.category.url +
          '">' +
          item.category.name + 
          '</a>'
...
function filter() {
  ...
  jQuery.post(
    "/wp-admin/admin-ajax.php",
    {
      action: "cases_filter_new",
      pp1: p1,
      pp2: p2,
    },
    function (data) {
		currentPageCase = 1;
		$('#load-more_cases').show();
      $("#fill").html(data);
...
//     const refs = document.getElementById("reffer").value;
    ...
    if (lang === en) {
      if (emailRegExp.test(email.value) && name.value && confirm.checked && msg.value && company.value) {
        handleSendEmail();
      }
    } else {
      [...fields].forEach(function (field) {
...
$('#load-more').on('click', function() {
    ...
    data: {
      ...
      paged: currentPage,
	  category: $('li.f-active').data('cat')
    },
    success: function (res) {
		if(currentPage >= res.max) {
		  $('#load-more').hide();
		}
      $('.blog-posts_loader').append(res.html);
    }
  });
});

// load more for cases
var currentPageCase = 1;
$('#load-more_cases').on('click', function() {
	currentPageCase++;
	$.ajax({
    type: 'POST',
    url: '/wp-admin/admin-ajax.php',
    dataType: 'json',
    data: {
      action: 'cases_load_more',
      paged: currentPageCase,
	  pp2: $('.block-inputs.selected').data('id')
    },
    success: function (res) {
		if(currentPageCase >= res.max) {
		  $('#load-more_cases').hide();
		}
      $('.case-wrap').append(res.html);
    }
  });
});